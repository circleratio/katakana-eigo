...
## Phase 9: 要件変更対応(ハイフン複合語・英数字混在語・UTF-8デコードエラー処理) ✅ 完了

`requirement.md`/`spec.md` の更新(ハイフン複合語・英数字混在語を1単語として扱う、UTF-8デコードエラーをエラーとして扱う)を実装に反映した。Phase 2 (`tokenizer.py`)・Phase 5 (`io_utils.py`)・Phase 6 (`cli.py`) を修正。

- [x] `tokenizer.py`: 単語判定の正規表現を `[A-Za-z]+(?:'[A-Za-z]+)*` から `[A-Za-z0-9]+(?:['-][A-Za-z0-9]+)*` に変更(spec.md 3.3節)
  - [x] マッチした文字列にアルファベットが1文字も含まれない場合(例: `123-456`)は `is_word=False` とする判定を追加。非単語トークンは前後の非単語文字列とまとめて1トークンに結合するよう `tokenize` の実装をバッファ方式に変更
  - [x] テスト追加(`tests/test_tokenizer.py`): `well-known`, `state-of-the-art`(ハイフン複合語) / `g2p`, `COVID-19`(英数字混在語) / `123-456`(数字のみ→非単語) / 数字のみの候補が前後の非単語文字列と1トークンに結合されること(`call 123-456!`, `123 456`) / 文中のダッシュ記号(`wait - what`、単語に取り込まれないこと) / ラウンドトリップ
- [x] `io_utils.py`: `_force_utf8` で `errors="strict"` を明示し、`open()` にも `errors="strict"` を指定。標準入力・ファイルいずれもUTF-8として不正なバイト列があれば `UnicodeDecodeError` が自然に送出されることを確認(spec.md 3.2節)
  - [x] テスト追加(`tests/test_io_utils.py`): 不正なUTF-8バイト列を含む標準入力(`TextIOWrapper` + `BytesIO`)・一時ファイルを読み込んだ場合に `UnicodeDecodeError` が送出されること
- [x] `cli.py`: `except UnicodeDecodeError` 分岐を追加し、`args.file is None` なら `エラー: 標準入力をUTF-8として読み込めません`、指定ファイルなら既存の `エラー: ファイルを読み込めません: <path>` を標準エラー出力へ出力して終了コード1で終了(spec.md 6章)
  - [x] テスト追加(`tests/test_cli.py`): 標準入力・ファイルそれぞれのデコードエラー時のメッセージ・終了コードを検証
- [x] `tests/test_integration.py` / `tests/fixtures/hyphen_and_alnum/`: ハイフン複合語・英数字混在語・数字のみの文字列を含む結合テストケースを追加(ゴールデンファイルは実行結果をそのまま採用)
- [x] `README.md`: 頭字語(`NASA`, `USA` 等)に関する記述を、単なる未対応ではなく大文字小文字を区別しない設計に起因する恒常的な限界である旨に更新(spec.md 8章に合わせる)

**完了条件**: 上記修正後に `pytest` が全件通過すること。`echo "well-known g2p COVID-19" | katakana-eigo` で複合語・英数字混在語が分割されず1単語として変換されること、不正なUTF-8入力でエラーメッセージと終了コード1が返ること。→ `pytest` 94件通過(既存76件+本フェーズ追加18件)。`printf 'hello \xff world' | python -m katakana_eigo.cli` で `エラー: 標準入力をUTF-8として読み込めません`(終了コード1)、同内容のファイルを指定すると `エラー: ファイルを読み込めません: <path>`(終了コード1)を実際に確認済み。

## Phase 10(将来対応・スコープ外)
- [ ] 出力先ファイル指定オプション(`-o`)
- [ ] 大きな入力に対するストリーミング処理
- [ ] `g2p_en` 依存が重い場合の軽量フォールバックルールの検討
- [ ] 略語(`NASA`, `USA` 等)の個別読み上げルールの検討 — 実現するには大文字小文字を区別しない現行設計を変更し、原文の大文字小文字情報を保持した判定ロジックを追加する必要がある(spec.md 8章参照)
...
Phase 0 環境構築
   ...
   └─ Phase 5 io_utils ──┘                                    ├─ Phase 6 cli(統合)
                                                                └─ Phase 7 結合テスト
                                                                     └─ Phase 8 ドキュメント
                                                                          └─ Phase 9 要件変更対応(tokenizer/io_utils/cli修正)
```

Phase 1〜5 は互いに依存が薄く並行して着手できるが、Phase 6 (CLI統合) は Phase 2/3/4/5 すべてに依存するため最後にまとめる。Phase 9 は Phase 2/5/6 で実装済みのモジュールに対する修正のため、それらの完了後に着手する。
